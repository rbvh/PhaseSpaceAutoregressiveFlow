...
// Process: e+ e- > t t~ > b b~ e- ve~ u d~ WEIGHTED<=12 @1
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_1_epem_ttx_bbxemvexudx(); 
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV2_3(w[5], w[4], pars->GC_100, pars->mdl_MW, pars->mdl_WW, w[9]); 
  FFV2_3(w[7], w[6], pars->GC_100, pars->mdl_MW, pars->mdl_WW, w[10]); 
  FFV2_1(w[2], w[10], pars->GC_100, pars->mdl_MT, pars->mdl_WT, w[11]); 
  FFV2_2(w[3], w[9], pars->GC_100, pars->mdl_MT, pars->mdl_WT, w[12]); 
  FFV2_4_3(w[1], w[0], -pars->GC_51, pars->GC_59, pars->mdl_MZ, pars->mdl_WZ,
      w[13]);

  // Calculate all amplitudes
  // Amplitude(s) for diagram number 0
  FFV1_0(w[12], w[11], w[8], pars->GC_2, amp[0]); 
  FFV2_5_0(w[12], w[11], w[13], pars->GC_51, pars->GC_58, amp[1]); 

}
double CPPProcess::matrix_1_epem_ttx_bbxemvexudx() 
{
  int i, j; 
  // Local variables
  const int ngraphs = 2; 
